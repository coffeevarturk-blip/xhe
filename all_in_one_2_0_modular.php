...
foreach ($accounts as $acc) {
    ...
    // ---- MODULE device_status (machine online/offline) ----
    if (!empty($CFG['modules']['device_status'])) {
        // uses device export CSV (/device?...&export=1) and sends TG only on ONLINE->OFFLINE transition
        $accName = (string)($acc['name'] ?? $acc['key'] ?? ('user_'.$userId));
        device_status_run_tr($accName, $userId, $CFG);
    }
