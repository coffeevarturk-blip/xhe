...
if (!function_exists('cleaning_ctrl_norm_type')) {
    function cleaning_ctrl_norm_type(string $s): string {
        $s = trim($s);
        // normalize "ES Brewer 1 (Quick Rinsing)" vs "ES Brewer 1(Quick Rinsing)"
        $s = preg_replace('/\s+\(/', '(', $s);
        return $s;
    }
}
...
if (!function_exists('cleaning_ctrl_parse_csv')) {
    function cleaning_ctrl_parse_csv(string $csv): array {
        ...
        // Use fgetcsv on in-memory stream to correctly parse commas/quotes
        $fp = fopen('php://temp', 'r+');
        fwrite($fp, $csv);
        rewind($fp);
        ...
        while (($data = fgetcsv($fp, 0, ",")) !== false) {
            if ($header === null) {
                if (isset($data[0])) $data[0] = preg_replace('/^\xEF\xBB\xBF/', '', $data[0]); // BOM
                $header = $data;
                continue;
            }
            $row = [];
...
if (!function_exists('cleaning_ctrl_run_for_account')) {
    function cleaning_ctrl_run_for_account(array $acc, int $userId, string $cookieStr, string $chatId, array &$notifyState): void {
        ...
        // Daily gate per account
        $gateKey = 'cleaning_control_' . $userId;
        if (!cleaning_ctrl_daily_should_run($gateKey, 86400)) {
            if (function_exists('xhe_log')) xhe_log('cleaning', "[cleaning][DEBUG] SKIP daily_gate user_id={$userId}", 'INFO');
            return;
        }
        ...
        if ($csv === null || strlen($csv) < 50) {
            if (function_exists('xhe_log')) xhe_log('cleaning', "[cleaning][ERROR] DOWNLOAD failed user_id={$userId} http={$http}", 'INFO');
            cleaning_ctrl_daily_mark_run($gateKey);
            return;
        }

        // Save CSV to export folder (optional but useful)
        $base = function_exists('runtime_base_dir') ? runtime_base_dir() : 'C:\\jetinno_runtime';
        ...
        if (empty($rows)) {
            if (function_exists('xhe_log')) xhe_log('cleaning', "[cleaning][ERROR] CSV parse empty user_id={$userId} http={$http}", 'INFO');
            cleaning_ctrl_daily_mark_run($gateKey);
            return;
        }
        ...
        foreach ($byVmc as $vmc => $list) {
            ...
            foreach ($requiredNorm as $idx => $needNorm) {
                ...
                foreach ($list as $rr) {
                    $ct = cleaning_ctrl_norm_type((string)($rr['Cleaning Type'] ?? ''));
                    if ($ct !== $needNorm) continue;
                    $ts = strtotime((string)($rr['Create Time'] ?? ''));
                    if ($ts > $last) $last = $ts;
                }
            ...
            if (!empty($missing)) {
                $viol[] = ['vmc'=>$vmc,'addr'=>$addr,'missing'=>$missing];
            }
        }
        ...
        if (!empty($viol)) {
            ...
            foreach ($viol as $v) {
                ...
                $text .= "\n";
                foreach ($v['missing'] as $m) $text .= "• {$m}\n";
                $text .= "\n";
            }

            // prefer per-account chat if present
            $targetChat = (string)($acc['telegram_chat_id'] ?? $chatId);
            if ($targetChat !== '' && function_exists('tg_notify')) {
                // key per account to avoid repeats; daily gate already prevents spam
                tg_notify('cleaning', $text, $targetChat, "cleaning_control|{$userId}", $notifyState);
            }
        } else {
            if (function_exists('xhe_log')) xhe_log('cleaning', "[cleaning][INFO] OK user_id={$userId} devices=" . count($byVmc), 'INFO');
        }
...
if ($enabled) {
    if (function_exists('xhe_log')) xhe_log('cleaning', '[cleaning][INFO] START module for account='.(string)($acc['key'] ?? $acc['name'] ?? 'ACC').' user_id='.(string)$userId, 'INFO');
