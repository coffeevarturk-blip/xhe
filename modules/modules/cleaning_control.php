...
if (!function_exists('cleaning_ctrl_run_for_account')) {
    function cleaning_ctrl_run_for_account(array $acc, int $userId, string $cookieStr, string $chatId, array &$notifyState): void {
        ...
        if (!cleaning_ctrl_daily_should_run($gateKey, 86400)) {
            if (function_exists('log_msg')) log_msg("[cleaning][DEBUG] SKIP daily_gate user_id={$userId}");
            return;
        }
        ...
        if ($csv === null || strlen($csv) < 50) {
            if (function_exists('log_msg')) log_msg("[cleaning][ERROR] DOWNLOAD failed user_id={$userId} http={$http}");
        ...
        if (empty($rows)) {
            if (function_exists('log_msg')) log_msg("[cleaning][ERROR] CSV parse empty user_id={$userId} http={$http}");
            cleaning_ctrl_daily_mark_run($gateKey);
            return;
        }
        ...
        if (!empty($viol)) {
            ...
            if (function_exists('log_msg')) log_msg("[cleaning][INFO] OK user_id={$userId} devices=" . count($byVmc));
        }
...
if ($enabled) {
    if (function_exists('log_msg')) log_msg('[cleaning][INFO] START module for account='.(string)($acc['key'] ?? $acc['name'] ?? 'ACC').' user_id='.(string)$userId);
